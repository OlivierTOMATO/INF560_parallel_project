...
void decision_1(int rank, int N, int nb_patterns, char *filename, int approx_factor, int n_bytes, int *n_matches, char **pattern, char *buf)
{
    ...
    if (rank == 0)
    {
        printf("MPI ONLY: Each pattern is handled by different MPI ranks, this is useful when No. of Ranks (%d) < (%d) No. of Patterns\n", N, nb_patterns);
...
int main(int argc, char **argv)
{
    ...
    printf("Sequential Pattern Matching code\n");
    printf("Approximate Pattern Mathing: "
           "looking for %d pattern(s) in file %s w/ distance of %d\n",
           nb_patterns, filename, approx_factor);
...
#pragma omp parallel
        ...
        else
        {
            ...
            else
            {
                if (N < 16)
                {
                    printf("%s\n", "OK, it seems that you don't have enough ranks, let's use static mpi");
                    if (OMP)
                    {
                        decision = 10;
                    }
                ...
                {
                    printf("%s\n", "OK, it seems that you have enough ranks, we plan to divide your test database into small chunks and allocate them to the processes");
                    if (n_bytes > 12582912)
                    {
                    ...
                    printf("%s\n", "You must want to know how we do implement our mpi code. I won't tell you! But remember that we are using static mpi, proven best by our experiments");
                    printf("%s\n", "we have two implementation of openmp honestly, typically opennmp over pattern or chunck, but we simply chose over chunk for you!");
                }
            }
        }
